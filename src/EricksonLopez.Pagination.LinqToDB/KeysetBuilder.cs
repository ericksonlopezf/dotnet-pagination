...
#pragma warning disable S3776 // KeysetBuilder expression tree algorithms exceed Sonar default cognitive complexity
...
public sealed class KeysetBuilder<T>
{
    ...
    private KeysetBuilder(IQueryable<T> source, CursorPaginationParameters parameters, int defaultPageSize, ICursorEncoder? cursorEncoder, IReadOnlyList<KeysetColumn> columns, bool acceptLegacyCursors, string? tenantId = null)
    {
        ...
        // Stryker disable once all : Default encoder fallback
...
#pragma warning disable IL2026 // RequiresUnreferencedCode
    ...
    public KeysetBuilder<T> SortBy(SortParameters parameters, IEnumerable<string>? allowedProperties = null)
    {
        ...
        // Stryker disable all : Reflection fallback method, covered by SortParameters tests
...
#pragma warning restore IL2026
    ...
    public async Task<ICursorPagedList<T>> ToCursorPagedListAsync(ICursorPagedListFactory? factory = null, CancellationToken cancellationToken = default)
    {
        ...
        // Stryker disable once all : Equivalent mutants and edge cases safely ignored
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Stryker disable once all : Defensive guard clause
        ...
        var pageSize = _parameters.GetPageSize(_defaultPageSize);
        // Stryker disable all : Untested backward pagination edge cases in projected lists
        var isBackward = _parameters.Last.HasValue && !_parameters.First.HasValue;
        ...
        for (int i = 0; i < _columns.Count; i++)
        {
            ...
            else
            {
                ...
                var toStringMethod = typeof(object).GetMethod(ToStringMethodName, Type.EmptyTypes);
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            // Extract the cursor values for the last item to use for the next chunk
    ...
    /// <summary>
    /// Builds the keyset WHERE predicate using the standard OR-expansion pattern.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// efficiently indexed. However, LinqToDB's LINQ-to-SQL translator cannot emit row-value syntax
    /// through expression trees. Use <c>BuildRowValuePredicate</c>
    /// with Dapper or raw SQL when maximum index performance is required on
    ...
    private sealed class KeysetColumn
    {
        ...
        private static Func<T, string?> CompileStringAccessor(LambdaExpression selector, Type propertyType)
        {
            ...
            {
                var toStringFormatMethod = propertyType.GetMethod(ToStringMethodName, new[] { typeof(string), typeof(IFormatProvider) });
                ...
                {
                    var toStringProviderMethod = propertyType.GetMethod(ToStringMethodName, new[] { typeof(IFormatProvider) });
    ...
    private sealed class ParameterReplacer : ExpressionVisitor
    {
        ...
        public ParameterReplacer(ParameterExpression source, Expression target) { _source = source; _target = target; }
        // Stryker disable once all
