...
**Action Items**:
- [x] Promote `LinqToDB`, `Redis`, `Relay`, `Elasticsearch`, and `Result` to `publish.yml`.
- [x] Remove `EricksonLopez.Pagination.SqlBuilder` (SQL generation consolidated into `EricksonLopez.Pagination.Dapper` via `CursorSqlBuilder` per ADR-0002; external query builder maintained in `dotnet-sql-builder`).

---

## Amendment: 2026-08-25 — Removal of SqlBuilder and Full Ecosystem Publishing

**Context**: `EricksonLopez.Pagination.SqlBuilder` was formally removed from the repository. The SQL building capabilities for Dapper are self-contained via `CursorSqlBuilder` within `EricksonLopez.Pagination.Dapper` (see ADR-0002), and general SQL building is delegated to the external `EricksonLopez.SqlBuilder` package. All 17 active packages in `/src/` are now published via `publish.yml`.
